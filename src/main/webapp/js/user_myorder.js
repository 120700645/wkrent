next_page_type = true;
//获取订单列表
function user_get_order(uid,page,status,showId) {
	loadingUpShow();
    var params = {};
    params.uid = uid;
    params.page_size = '10';
    ...
    connectserver("../common/dopost.html",postDate, function(db){
    	...
    	$("#"+showId).attr('code',1);
    	if(db.orders.length < 1 && page == 1 && status == 4){
    		hideDiv(showId);
    		$("#no_order_div").show();
    		loadingUpHide();
    	}else if(db.orders.length < 1 && page == 1 && status != 4){
    		hideDiv(showId);
    		...
    		if(db.orders.length == 0){
    			next_page_type = false;
    			return;
    		}
    		
    		if(db.orders.length <10){
    			next_page_type = false;
    		}
...
//    				order_tr += '<i>'+orders.city+'</i> ';
    				...
    				order_tr += '</td>';
    				order_tr += '</tr>';
    				$("#"+showId+"Tbody").append(order_tr);
    		}
    		loadingUpHide();
    		$("#"+showId+"Fenye").text(page);
...
//隐藏DIV
function hideDiv(showId){
	$("#no_order_div").hide();
	$("#no_type_order_div").hide();
	$(".personal_right_order").hide();
	$("#"+showId+"Div").hide();
}

//上一页
function last_page(type){
	var page = ($("#"+type+"Fenye").text()-1);
	var status = $("#"+type).attr("num");
	if(page < 1){
		return;
	}
	next_page_type = true;
	user_get_order(uid,page,status,type);
	javascript:scroll(0,0);
}

//下一页
function next_page(type){
	var page = (Number($("#"+type+"Fenye").text())+1);
	var status = $("#"+type).attr("num");
	if(next_page_type){
		user_get_order(uid,page,status,type);
		javascript:scroll(0,0);
